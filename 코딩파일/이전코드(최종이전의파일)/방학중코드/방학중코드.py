...
class RequestHandler_httpd(BaseHTTPRequestHandler):
 def do_GET(self):
  ...
  if Request == 'jadong': #어플에서 자동모드 실행시
            ...
            mylcd.lcd_display_string("{0:.1f}C {1}cm".format(watertemp, lvwater),1)
            mylcd.lcd_display_string("Lux={0}Lux, value={1}".format(llux, pol),2) #LCD에 조도센서값과 수온센서값 탁도센서값 표시
  ...
  if Request == 're' : #어플에서 수동모드 실행시
             ...
             mylcd.lcd_display_string("{0:.1f}C {1}cm".format(value2, value),1)
             mylcd.lcd_display_string("{0}Lux value={1}".format(value1, value3),2) #LCD에 조도센서값과 수온센서값 탁도센서값 표시
             bus.write_byte(address,AIN3)
             bus.read_byte(address)
