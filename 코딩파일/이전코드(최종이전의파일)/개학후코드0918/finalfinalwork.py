...
from flask import Flask, request, jsonify, render_template, Response #WAS 생성 라이브러리
from werkzeug.utils import redirect #웹 애플리케이션에서 HTTP 리디렉션을 수행하기 위해 사용되는 함수
import Adafruit_DHT
from PIL import ImageFont, ImageDraw, Image #이미지 처리와 조작을 위한 파이썬 라이브러리
import datetime
...
mylcd = I2C_LCD_driver.lcd() #LCD 라이브러리
...
@app.route('/')
def index():
    global value, value1, value2, value3, temperature, humidity, jodo, takdo
...
def jadong():
    ...
    if humidity is not None and temperature is not None:
        print("온도={0:0.1f}°C 습도={1:0.1f}%".format(temperature, humidity)) #온습도 센서 값 출
    ...
    if lvwater < 10:
          ...
          if value == 1:
                GPIO.output(13, 0)
                value = 0 # 수위센서 범위를 임의로 지정하여 물공급 모터 구동 함수
    #물 공급 자동으로 하면 깔끔한 느낌이 안남 수정필요
    return redirect('/')                
...
def stop():
    ...
    GPIO.output(20, 0)
    GPIO.output(13, 0)
    GPIO.output(19, 0) #모든 GPIO에 연결된 모터, LED 정지
    return redirect('/')
...
def saryon():  #어플에서 사료 모터 on
    ...
    return redirect('/')
@app.route('/saryooff', methods=['GET'])
def saryooff():  #어플에서 사료 모터 of
