...
#1017 수정GPIO.setup(16, GPIO.OUT) #사료모터
#1017 수정GPIO.setup(20, GPIO.OUT) #사료모터
...
#1017 수정motor_pwm_pin_1 = 16 # 사료모터 회전속도 조절 
#1017 수정GPIO.setup(motor_pwm_pin_1, GPIO.OUT)
#1017 수정motor_pwm_1 = GPIO.PWM(motor_pwm_pin_1, 1000)
...
#1017 수정def set_motor_speed_1(speed):
#1017 수정    motor_pwm_1.start(speed)
...
#탁도, 조도, 수위센서 환경에 따라 조절해서 사용하세요. 0920 수정   
    if pol < 60:
        if takdo != 1:
            GPIO.output(5,1)            
    ...
    if llux > 228: #0927 FIX
        if jodo != 1:
            GPIO.output(26, 1)
            jodo = 1
    elif llux < 224: #0927 FIX
        if jodo == 1:
            GPIO.output(26, 0)
    ...
    #물 공급 자동으로 하면 깔끔한 느낌이 안남 수정필요 / 0926 수정완료
    return redirect('/')                
...
def stop():
    ...
    #motor_pwm_1.stop()#0927 FIX
    motor_pwm_2.stop()
    motor_pwm_3.stop()
    #자동 모드에서 나왔을 시 다시 자동모드가 작동 되게 해줌 1004 수정
    global value, value1, value2, value3, temperature, humidity, jodo, takdo, suwui
    value = 0
    value1 = 0
    value2 = 0
